...
**Hierarchical Clustering (Phân cụm phân cấp)** xây dựng cây phân cấp của các cụm.
...
```{r hierarchical-process, fig.height=8}
set.seed(42)
...
- Công thức: $d(A, B) = \frac{1}{|A||B|} \sum_{a \in A, b \in B} d(a, b)$
...
```{r linkage-illustration, fig.height=6}
par(mfrow = c(2, 2))
...
```{r dendrogram-example, fig.height=8}
...
```{r hierarchical-elbow, fig.height=6}
# Tính WSS cho mỗi K
...
```{r hierarchical-full-example, fig.height=8}
# Chọn K = 3
...
**Ý tưởng cốt lõi:** Tìm cụm dựa trên **mật độ**, không phải khoảng cách.
...
```{r dbscan-point-types, fig.height=6}
set.seed(42)
...
```{r dbscan-parameters, fig.height=8}
library(dbscan)
...
```{r dbscan-algorithm, fig.height=10}
set.seed(42)
...
```{r dbscan-outliers, fig.height=5}
# Dữ liệu có outliers
set.seed(456)
...
```{r dbscan-real-example, fig.height=8}
...
```{r comparison-three-methods, fig.height=5}
par(mfrow = c(1, 3))
...
**Dữ liệu:** 300 khách hàng với 3 đặc điểm:
- Tuổi (Age)
...
```{r project-data-viz, fig.height=8}
par(mfrow = c(2, 2))
...
```{r kmeans-elbow-project, fig.height=6}
...
```{r kmeans-viz-project, fig.height=8}
par(mfrow = c(2, 2))
...
```{r hierarchical-dendrogram-project, fig.height=7}
...
```{r hierarchical-viz-project, fig.height=8}
par(mfrow = c(2, 2))
...
```{r dbscan-knn-project, fig.height=6}
library(dbscan)
...
```{r dbscan-viz-project, fig.height=8}
par(mfrow = c(2, 2))
...
```{r comparison-visual-project, fig.height=10}
par(mfrow = c(3, 3))
...
```{r silhouette-viz-project, fig.height=8}
par(mfrow = c(3, 1))
...
Tỷ lệ khoảng cách nhỏ nhất giữa cụm / khoảng cách lớn nhất trong cụm (càng cao càng tốt)
...
```{r cluster-strategies, echo=FALSE, results='asis'}
cat("\n**Cụm 1:**", cluster_profiles$So_luong[1], "khách hàng\n\n")
cat("- Tuổi TB:", cluster_profiles$Tuoi_TB[1], "tuổi\n")
cat("- Thu nhập TB:", cluster_profiles$Thu_nhap_TB[1], "triệu/tháng\n")
cat("- Spending Score:", cluster_profiles$Spending_TB[1], "/100\n")

if(cluster_profiles$Tuoi_TB[1] < 30) {
  cat("- **Phân loại**: Nhóm SINH VIÊN / TRẺ TUỔI\n")
  cat("- **Chiến lược**:\n")
  cat("  - Sản phẩm giá rẻ, chất lượng tốt\n")
  cat("  - Khuyến mãi mạnh, flash sale\n")
  cat("  - Marketing qua TikTok, Instagram\n")
  cat("  - Chương trình student discount\n\n")
} else if(cluster_profiles$Tuoi_TB[1] > 60) {
  cat("- **Phân loại**: Nhóm CAO TUỔI\n")
  cat("- **Chiến lược**:\n")
  cat("  - Sản phẩm chất lượng, bền vững\n")
  cat("  - Dịch vụ tận tâm, chu đáo\n")
  cat("  - Chương trình khách hàng thân thiết\n")
  cat("  - Hỗ trợ tư vấn cá nhân\n\n")
} else {
  cat("- **Phân loại**: Nhóm TRUNG NIÊN\n")
  cat("- **Chiến lược**:\n")
  cat("  - Sản phẩm cao cấp, premium\n")
  cat("  - Chương trình VIP, loyalty\n")
  cat("  - Dịch vụ giao hàng nhanh\n")
  cat("  - Tư vấn chuyên sâu\n\n")
}

cat("---\n\n")
cat("**Cụm 2:**", cluster_profiles$So_luong[2], "khách hàng\n\n")
cat("- Tuổi TB:", cluster_profiles$Tuoi_TB[2], "tuổi\n")
cat("- Thu nhập TB:", cluster_profiles$Thu_nhap_TB[2], "triệu/tháng\n")
cat("- Spending Score:", cluster_profiles$Spending_TB[2], "/100\n")

if(cluster_profiles$Tuoi_TB[2] < 30) {
  cat("- **Phân loại**: Nhóm SINH VIÊN / TRẺ TUỔI\n")
  cat("- **Chiến lược**: Giá rẻ, khuyến mãi, social media\n\n")
} else if(cluster_profiles$Tuoi_TB[2] > 60) {
  cat("- **Phân loại**: Nhóm CAO TUỔI\n")
  cat("- **Chiến lược**: Chất lượng, dịch vụ, khách hàng thân thiết\n\n")
} else {
  cat("- **Phân loại**: Nhóm TRUNG NIÊN\n")
  cat("- **Chiến lược**: Cao cấp, VIP, dịch vụ đặc biệt\n\n")
}

cat("---\n\n")
cat("**Cụm 3:**", cluster_profiles$So_luong[3], "khách hàng\n\n")
cat("- Tuổi TB:", cluster_profiles$Tuoi_TB[3], "tuổi\n")
cat("- Thu nhập TB:", cluster_profiles$Thu_nhap_TB[3], "triệu/tháng\n")
cat("- Spending Score:", cluster_profiles$Spending_TB[3], "/100\n")

if(cluster_profiles$Tuoi_TB[3] < 30) {
  cat("- **Phân loại**: Nhóm SINH VIÊN / TRẺ TUỔI\n")
  cat("- **Chiến lược**: Giá rẻ, khuyến mãi, social media\n\n")
} else if(cluster_profiles$Tuoi_TB[3] > 60) {
  cat("- **Phân loại**: Nhóm CAO TUỔI\n")
  cat("- **Chiến lược**: Chất lượng, dịch vụ, khách hàng thân thiết\n\n")
} else {
  cat("- **Phân loại**: Nhóm TRUNG NIÊN\n")
...
4. **Outliers**: DBSCAN phát hiện được outliers, nhưng mất một số điểm dữ liệu

**KHUYẾN NGHỊ:**

✅ **Nên chọn K-MEANS** vì:

- Silhouette Score cao nhất: **`r round(avg_sil_kmeans, 3)`**
- WSS thấp nhất: **`r round(wss_kmeans_val, 0)`** → Cụm chặt chẽ
- Dunn Index cao nhất: **`r round(dunn_kmeans, 3)`** → Cụm phân tách rõ
...
- Chậm hơn K-Means
- Kết quả tương tự K-Means

⚠️ **DBSCAN** không phù hợp vì:
- Silhouette thấp hơn
- Mất dữ liệu do coi là noise
- Khó chọn tham số eps

---

**KẾT LUẬN:** 

Với bài toán phân khúc khách hàng này, **K-Means với K=3** là lựa chọn tốt nhất, cho phép chia khách hàng thành 3 nhóm rõ ràng để áp dụng chiến lược marketing phù hợp.
